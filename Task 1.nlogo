...
to color-patch  ;;patch procedure
  ifelse on?
    [ set pcolor foreground set alive_cells alive_cells + 1]
...
to extrapolate-switches
  ...
  set lambda 0
  if ooi [set lambda lambda + 1]
  if oio [set lambda lambda + 1]
  if oii [set lambda lambda + 1]
  if ioo [set lambda lambda + 1]
  if ioi [set lambda lambda + 1]
  if iio [set lambda lambda + 1]
  if iii [set lambda lambda + 1]
  set lambda lambda / 7
  end
...
<experiments>
  ...
  <experiment name="2" repetitions="1" runMetricsEveryStep="true">
