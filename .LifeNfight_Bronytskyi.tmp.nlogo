...
to set_figure_3_r_pentomino [c n m]
...
to set_figure_4_two_glider_mess[c n m]
...
to set_figure_5_alien_ship_explosion[c n m]
...
to go
  ...
  ask patches [
    ...
    let alive_neighbors neighbors with [pcolor != white]
    let n_alive_neighbors count alive_neighbors

    ; if patch is dead
    ifelse pcolor = white [

      ; if we have 3 alive neighbors
      if n_alive_neighbors = 3 [

        set alive? true
        ;; find out collor
        let my-list [pcolor] of alive_neighbors
        ;show my-list
      ...
      ; show pxcor
      ; show pycor
      ; show count neighbors with [pcolor != white]
      if (n_alive_neighbors > 3) or (n_alive_neighbors < 2) [
        set alive? false
...
"" ""
PENS
"default" 1.0 0 -13345367 true "" "plot count patches with [pcolor = blue]"
"pen-1" 1.0 0 -2674135 true "" "plot count patches with [pcolor = red]"
"pen-2" 1.0 0 -16777216 true "" "plot count patches with [pcolor = black]"
"pen-3" 1.0 0 -10899396 true "" "plot count patches with [pcolor = green]"
